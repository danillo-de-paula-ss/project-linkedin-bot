...
class Program:
    ...
    def make_win(self) -> sg.Window:
        ...
        left_frame = [
            [sg.Text('Como deve ser feito o processo de login?'), sg.Radio('manual', 'login', key='-LOGIN-PROCESSa-', enable_events=True, default=True), sg.Radio('automático', 'login', key='-LOGIN-PROCESSb-', enable_events=True, default=False, disabled=True)],
    ...
    def main(self, nobrowser) -> None:
        ...
        while True:
            ...
            # import text file
                ...
                if path != '':
                    with open(path, 'rt', encoding='utf-8') as file:
                        self.window['-TEXT-TO-WRITE-'].update(file.read())
            elif event == '-CLEAN-ALL-':
                self.window['-TEXT-TO-WRITE-'].update('')
            ...
            # bot system
                ...
                thread_bot = threading.Thread(target=start_bot, args=(self.window, self.driver, self.wait, values['-KEY-MESSAGE-'], values['-TEXT-TO-WRITE-'], posts_list.index(values['-COMBO-POSTS-']), int(values['-SPIN-WAIT-'] * 60)), daemon=True)
                thread_bot.start()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-nb', '--nobrowser', action='store_false', help='does not launch the browser')
